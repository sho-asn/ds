...
import game.actors.fire.FireHorse;
...
/**
 * This class represents the SouthwestEnemy, that decides what enemies to spawn on the Southwest side of the map
...
public class SouthwestEnemy implements EnemyFactory {
    ...
    public void CreateFire(Location location) {
        if (RandomNumberGenerator.getRandomInt(100) <= 29) {
            location.addActor(new FireHorse());
        }
    }
}
