...
import game.actors.fire.FireGoat;
...
/**
 * This class represents the NortheastEnemy, that decides what enemies to spawn on the Northeast side of the map
 * @author Seah Thern Fong
 */
public class NortheastEnemy implements EnemyFactory {
    /**
    ...
    public void CreateFire(Location location) {
        if (RandomNumberGenerator.getRandomInt(100) <= 29) {
            location.addActor(new FireGoat());
        }
    }
}
